...
for (i in 1 : simulations) {
  ...
  #impute
  OUT[[i]] <- mice(incomplete.data, m = 5, print = FALSE)
...
format <- c("Mean" = 0, "Lower" = 0, "Upper" = 0)
results.MCSE <- EVAL %>% vapply(., info, format) %>% t()%>%as.data.frame()
results.MCSE$Mean %>% subtract(mean(results.MCSE$Mean)) %>% 
  raise_to_power(2) %>% sum() %>% divide_by(1000*999) %>% sqrt()
