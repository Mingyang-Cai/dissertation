...
for (i in 1:simulations){
  # Generate data
  y <- rnorm(sample.size, mean = 2, sd = 1)
  epsilon <- rnorm(sample.size, mean = 0, sd = 6)
  x <- 2*y + y^2 + epsilon
  ...
  meth <- c("quadratic", "~I(Y^2)", "")
  pred <- ini$pred
  pred[, "YY"] <- 0
  
  # Impute data
  imp.pc <- mice(dat, meth = meth, pred = pred, print = FALSE)
  
  summary.OUT <- list()
  for (j in 1:5) {
    summary.OUT[[j]] <- summary(lm(complete(imp.pc,j)$X~complete(imp.pc,j)$Y+complete(imp.pc,j)$YY))
  }
...
# Evaluate the coverage rate
evaluate.sims <- function(sims, truth = 2){
  POOL <- list()
  pb <- txtProgressBar(min = 0, max = simulations, style = 3)
  for (i in 1:length(sims)){
    #Extract means and variances
