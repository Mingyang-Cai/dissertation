...
for (i in 1:simulations){
  ...
  # Generate missings in X
  incomplete.data <- ampute(data = complete.data, prop = missingness, mech = "MCAR", patterns = c(1, 0, 0))$amp
  
  #incomplete.data <- ampute(data=complete.data, prop=missingness, mech = "MAR", type = 'LEFT', patterns = c(1, 0, 0),
  #                          weights = c(1, 0, 0))$amp
  colnames(incomplete.data) <- c("Y", "X", "XX")
  imp <- smcfcs(incomplete.data, smtype = "lm", smformula = "Y~X+XX", method = c("", "norm", "X^2"), rjlimit= 1000)
