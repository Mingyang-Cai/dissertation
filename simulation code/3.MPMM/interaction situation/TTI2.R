...
for (i in 1:simulations){
  ...
  imp <- mice(incomplete.data, m = 5, method = "norm",  print = FALSE)
  
  summary.OUT <- list()
  for (j in 1:5) {
    summary.OUT[[j]] <- summary(lm(complete(imp,j)$X~complete(imp,j)$Y+complete(imp,j)$Z+complete(imp,j)$YZ))
  }
