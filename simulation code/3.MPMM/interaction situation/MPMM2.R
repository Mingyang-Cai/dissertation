...
pmm.cra.single <- function(data){
  ...
  data.obs.y <- data.obs[1:3]
  data.obs.x <- data.obs[4]
  
  data.mis.y <- data.mis[1:3]
  data.mis.x <- data.mis[4]
  ...
  imp<-mice.impute.pmm(frv.total,!is.na(frv.total),rbind(data.obs.x, data.mis.x))
...
for (i in 1:simulations){
  ...
  incomplete.data <- ampute(data=complete.data, prop=missingness, mech = "MAR", type = 'RIGHT', patterns = c(0, 0, 0, 1),
  ...
  summary.OUT <- list()
  for (j in 1:5) {
    summary.OUT[[j]] <- summary(lm(OUT[[j]]$X~OUT[[j]]$Y+OUT[[j]]$Z+OUT[[j]]$YZ))
  }
