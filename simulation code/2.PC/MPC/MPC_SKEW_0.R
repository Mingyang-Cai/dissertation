...
MPC.function <- function(complete.data, missingness, m.mech){
  incomplete.data <- Ampute.function(complete.data, missingness, m.mech)
  ini <- mice(incomplete.data, maxit = 0)
  meth <- c("quadratic", "~I(X^2)", "")
  pred <- ini$pred
  pred[, "XX"] <- 0
  imp.mpc <- mice(incomplete.data, meth = meth, pred = pred, quad.outcome = "Y", print = FALSE)
  fit <- with(imp.mpc, lm(Y ~ X + XX))
...
for (i in 1 : simulations) {
  X <- rsn(sample.size, xi = 0, omega = 1, alpha = 6)
  ...
  MCAR[[i]] <-MPC.function(complete.data = complete.data, missingness = missingness, m.mech = "MCAR")
  MARl[[i]] <-MPC.function(complete.data = complete.data, missingness = missingness, m.mech = "MARleft")
  MARm[[i]] <-MPC.function(complete.data = complete.data, missingness = missingness, m.mech = "MARmid")
  MARt[[i]] <-MPC.function(complete.data = complete.data, missingness = missingness, m.mech = "MARtail")
  MARr[[i]] <-MPC.function(complete.data = complete.data, missingness = missingness, m.mech = "MARright")
